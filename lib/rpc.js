  ...
  http.request(requestOptions, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 204) {
        ...
        if (res.headers['Content-Type'].indexOf('colenc=B')) {
          callback(undefined, this._parseBase64Encode(ret, options.encode));
        } else if (res.headers['Content-Type'].indexOf('colenc=U')) {
          callback(undefined, this._parseURLEncode(ret));
        } else {
          callback(undefined, this._parse(ret));
        }
