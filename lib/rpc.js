  ...
  _parse(tsv, encoding) {
    ...
    if (typeof tsv === 'string' && tsv.length > 0) {
      ...
      if (typeof encoding === 'undefined' || encoding === null) {
        ...
        for (let i = 0; i < lines.length; i += 1) {
          const [k, v] = lines[i].split('\t');
          ret[k] = Buffer.from(v).toString(encoding);
        }
      }
    }
  ...
  _parseBase64Encode(tsv, encoding) {
    ...
    if (typeof tsv === 'string' && tsv.length > 0) {
      ...
      if (typeof encoding === 'undefined' || encoding === null) {
        for (let i = 0; i < lines.length; i += 1) {
          const [k, v] = lines[i].split('\t');
          ret[Buffer.from(k, 'base64').toString()] =
          Buffer.from(v, 'base64').toString();
        }
      } else if (encoding === 'binary') {
        for (let i = 0; i < lines.length; i += 1) {
          const [k, v] = lines[i].split('\t');
          ret[Buffer.from(k, 'base64').toString()] = Buffer.from(v, 'base64');
        }
      } else {
        for (let i = 0; i < lines.length; i += 1) {
          const [k, v] = lines[i].split('\t');
          ret[Buffer.from(k, 'base64').toString(encoding)] =
          Buffer.from(v, 'base64').toString(encoding);
        }
      }
    }
  ...
  _parseURLEncode(tsv, encoding) {
    ...
    if (typeof tsv === 'string' && tsv.length > 0) {
      ...
      if (typeof encoding === 'undefined' || encoding === null) {
        ...
        for (let i = 0; i < lines.length; i += 1) {
          const [k, v] = lines[i].split('\t');
          ret[decodeURIComponent(k)] = Buffer.from(decodeURIComponent(v));
        }
      } else {
        for (let i = 0; i < lines.length; i += 1) {
          const [k, v] = lines[i].split('\t');
